    ...
    <div className="min-h-screen safe-bottom" style={{ background: 'linear-gradient(180deg, #F2F2F7 0%, #E8E8ED 100%)' }}>
      <div className="max-w-md mx-auto px-5 py-4 space-y-6">
        <header className="text-center space-y-3 pt-2">
          <p className="text-[13px] font-medium tracking-wide" style={{ color: '#8E8E93' }}>{RUNWAY_KICKER}</p>
          <p className="text-[15px] leading-relaxed" style={{ color: '#6E6E73' }}>输入本金和常住地区，看看现金能为你换来多久的选择时间。</p>
        </header>

        <div className="sticky top-0 z-30 -mx-5 px-5 py-3 text-center backdrop-blur-md" style={{ background: 'rgba(242,242,247,0.92)', borderBottom: '1px solid rgba(0,0,0,0.06)' }}>
          <h1 className="text-[28px] font-bold tracking-tight" style={{ color: '#1C1C1E', letterSpacing: '-0.03em' }}>{RUNWAY_TITLE}</h1>
        </div>
        ...
        <div className="apple-card space-y-5">
          <div>
            ...
            <div className="relative"><span className="pointer-events-none absolute left-4 top-1/2 z-10 -translate-y-1/2 text-[16px] font-medium" style={{ color: '#8E8E93' }}>¥</span><input id="runway-cash" type="text" inputMode="decimal" className="apple-input" style={{ paddingLeft: '44px' }} value={cashInput} onChange={handleCashChange} aria-describedby={cashError ? 'cash-error' : 'cash-hint'} aria-invalid={!!cashError} /></div>
          ...
          <button type="button" onClick={handleCalculate} className="apple-btn apple-btn-primary w-full py-3.5 text-[17px]" style={{ boxShadow: '0 4px 16px rgba(0,122,255,0.3)' }}>测测这笔现金能过多久</button>
        </div>
        ...
        <div className="apple-card space-y-3">
          ...
          {showMethodology && <div className="space-y-4 pt-2 text-[13px] leading-relaxed" style={{ color: '#6E6E73' }}>
            {resultBaseline ? <div className="rounded-xl p-4 space-y-1.5" style={{ background: 'rgba(0,0,0,0.02)' }}><p className="font-semibold text-[#1C1C1E] mb-1">本次使用的数据</p><p>地区：{resultBaseline.sourceRegionName}（{resultBaseline.regionLevel === 'city' ? resultBaseline.regionGroup : resultBaseline.regionLevel === 'province' ? `${resultBaseline.regionGroup}代表省` : '全国'}）</p><p>属地口径：最低工资与城镇居民平均消费均取上述所选地区口径</p><p>年份：{resultBaseline.dataYear} 年；指标：{resultBaseline.metricName}</p><p>城镇居民平均消费年值：{resultBaseline.annualYuan.toLocaleString('zh-CN')} 元/人/年</p><p>最低工资：{resultBaseline.minimumWageMonthlyYuan.toLocaleString('zh-CN')} 元/月（最低档）</p><p>消费数据来源：{resultBaseline.sourceName}，发布日期 {resultBaseline.sourcePublishedAt}</p><p>最低工资来源：{resultBaseline.minimumWageSourceName}</p>{resultBaseline.dataNote && <p>备注：{resultBaseline.dataNote}</p>}<div className="flex gap-3 pt-1"><a href={resultBaseline.sourceUrl} target="_blank" rel="noopener noreferrer" className="text-[#007AFF] underline">查看消费官方来源</a><a href={resultBaseline.minimumWageSourceUrl} target="_blank" rel="noopener noreferrer" className="text-[#007AFF] underline">查看最低工资表</a></div></div> : <p style={{ color: '#8E8E93' }}>请先完成计算后查看本次数据来源。</p>}
            <div><p className="font-semibold mb-1" style={{ color: '#1C1C1E' }}>计算公式</p><div className="rounded-lg p-3 space-y-0.5" style={{ background: 'rgba(0,0,0,0.03)', fontFamily: 'monospace', fontSize: '12px' }}><p>躺平过 = 最低工资 × 50%</p><p>省着过 = 最低工资 × 100%</p><p>正常过 = 城镇居民平均消费年值 ÷ 12</p><p>从容过 = 城镇居民平均消费 × 130%</p><p>奢侈过 = 城镇居民平均消费 × 300%</p><p>能过月数 = floor(可自由支配现金 ÷ 本档月消费额)</p></div></div>
            <div><p className="font-semibold mb-1" style={{ color: '#1C1C1E' }}>口径说明</p><ul className="list-disc pl-4 space-y-1"><li>城市档覆盖确认的一线、新一线城市；这些城市的情景默认仍需工作，计算只表示现金覆盖时间。</li><li>省级档按华东（浙江、江苏）、西北（甘肃、陕西）、东北（吉林）、西南（四川、贵州）、华南（广东、广西）、华中（山东、河南、湖南）选择代表省。</li><li>最低工资和城镇居民平均消费均落在所选属地口径：选城市用城市数据，选代表省用省级均值，选全国用全国城镇均值。</li><li>所有消费数据取 2025 年年度公开统计值；最低工资取 2025 年最低档。</li><li>全国最低工资为各省 2025 年最低工资标准按城镇人口加权估算，不是国家统一最低工资。</li><li>休息对比采用 PFOS 场景基准：全国城镇职工加权平均全年实际休息中位值约 100 天，不是对每个人的法定休息承诺。</li></ul></div>
